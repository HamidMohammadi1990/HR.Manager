...
public class GetLeaveRequestHandler
    ...
    (ILeaveRequestRepository leaveRequestRepository,
     ...
     ILeaveRequestMapperService mapper)
...

{
...

    {
        ...
        var approvalSteps = await approvalStepRepository.GetByLeaveRequestIdAsync(leaveRequest.Id, cancellationToken);



        var leaveRequestWithSteps = await leaveRequestRepository.FindWithApprovalStepsAsync(leaveRequest.Id, cancellationToken);

        var canCurrentUserAct = leaveRequestWithSteps is not null

            && await leaveApprovalWorkflowService.CanUserActOnCurrentStepAsync(

                leaveRequestWithSteps,
