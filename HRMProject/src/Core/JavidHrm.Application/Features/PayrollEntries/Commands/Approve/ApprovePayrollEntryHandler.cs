...
public class ApprovePayrollEntryHandler
    (IPayrollEntryRepository payrollEntryRepository, IUnitOfWork uow)
    : IRequestHandler<ApprovePayrollEntryRequest, OperationResult>
{
    public async Task<OperationResult> Handle(ApprovePayrollEntryRequest request, CancellationToken cancellationToken)
    {
        ...
        if (payrollEntry.Status != PayrollEntryStatus.Draft)
            return ErrorModel.Create(MessageKeys.PayrollEntryNotDraft);

        payrollEntry.Approve();
